...
/*******************************************************************************
* Draw a line in Two top oncants only bc this function wasn't provided?? :/ :/ *
*   Parameter:      octant: the octant                                    *
*   Return:                                                                    *
*******************************************************************************/
void GLCD_DrawLine(int octant,int startX, int startY, int endX, int endY) {
	int d_X, d_Y, A, B, P, i, curr_x, curr_y;

	d_X = abs(endX - startX);
	d_Y = abs(endY - startY);
	
	GLCD_SetWindow(startX - abs(d_X),startY - abs(d_Y), 2*d_X, 2*d_Y);
	
	if (octant == 6){
		A = 2*d_X;
		B = A - 2*d_Y;
		P = A - d_Y;
		GLCD_PutPixel(curr_x, curr_y);
		curr_x = startX;
		curr_y = startY;
		for(i = 0; i < d_Y; i++){
			if (P < 0){
